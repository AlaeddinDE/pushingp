...
$stmtV2 = $mysqli->prepare('SELECT id, pin_plain, flag, status FROM members_v2 WHERE name=? LIMIT 1');
if ($stmtV2) {
...
if ($memberId === null) {
  $stmtLegacy = $mysqli->prepare('SELECT id, pin, flag, is_locked FROM members WHERE name=? LIMIT 1');
  if (!$stmtLegacy) {
...
$stmtAdminV2 = $mysqli->prepare('SELECT is_admin FROM admins_v2 WHERE member_id=? LIMIT 1');
if ($stmtAdminV2) {
  ...
  $stmtAdminLegacy = $mysqli->prepare('SELECT 1 FROM admins WHERE member_name=? OR pin=? LIMIT 1');
  if ($stmtAdminLegacy) {
