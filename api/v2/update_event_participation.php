...
$stmt = $mysqli->prepare('SELECT 1 FROM events WHERE id=? AND status!=\'canceled\'');
if (!$stmt) {
...
$stmt = $mysqli->prepare('INSERT INTO event_participants (event_id, member_id, state, availability) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE state=VALUES(state), availability=VALUES(availability), updated_at=CURRENT_TIMESTAMP');
if (!$stmt) {
