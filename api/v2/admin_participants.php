...
$stmt = $conn->prepare('SELECT ep.member_id, m.name, ep.state, ep.availability, ep.updated_at FROM event_participants ep LEFT JOIN members_v2 m ON m.id = ep.member_id WHERE ep.event_id=? ORDER BY m.name ASC');
if (!$stmt) {
