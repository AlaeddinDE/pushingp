...
$stmt = $conn->prepare('SELECT title, description, start, end, location, COALESCE(cost,0), paid_by, status FROM events WHERE id=?');
if (!$stmt) {
...
$stmt = $conn->prepare('UPDATE events SET title=?, description=?, start=?, end=?, location=?, cost=?, paid_by=?, status=? WHERE id=?');
if (!$stmt) {
...
if ($status === 'canceled') {
    $stmt = $conn->prepare('UPDATE reservations_v2 SET status="cancelled" WHERE event_id=? AND status="active"');
    if ($stmt) {
        $stmt->bind_param('i', $eventId);
        $stmt->execute();
        $stmt->close();
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->fetch()) {
                $stmt->close();
                $stmt = $conn->prepare('UPDATE reservations_v2 SET amount=?, status="active" WHERE id=?');
                if ($stmt) {
...
api_log_admin_action($conn, api_current_user_id() ?? 0, 'admin_update_event', 'event', (string) $eventId, $data);
