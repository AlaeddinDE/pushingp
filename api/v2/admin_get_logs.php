...
$total = 0;
$stmt = $mysqli->prepare('SELECT COUNT(*) FROM admin_logs');
...
$stmt = $mysqli->prepare('SELECT l.id, l.admin_id, m.name, l.action, l.entity_type, l.entity_id, l.payload_hash, l.details, l.created_at FROM admin_logs l LEFT JOIN members_v2 m ON m.id = l.admin_id ORDER BY l.created_at DESC LIMIT ?, ?');
if (!$stmt) {
