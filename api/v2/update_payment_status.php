...
if ($status === 'paid' && $currentStatus !== 'paid') {
    ...
    $stmt = $mysqli->prepare('SELECT 1 FROM transactions_v2 WHERE payment_request_id=? LIMIT 1');
    if ($stmt) {
        $stmt->bind_param('i', $requestId);
        $stmt->execute();
        $stmt->store_result();
...
api_log_admin_action($mysqli, api_current_user_id() ?? 0, 'update_payment_status', 'payment_request', (string) $requestId, [
    'status' => $status,
]);
