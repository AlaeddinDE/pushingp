...
$stmt = $mysqli->prepare('SELECT title, description, start, end, location, COALESCE(cost,0), paid_by, created_by, status FROM events WHERE id=?');
if (!$stmt) {
...
$stmt = $mysqli->prepare('UPDATE events SET title=?, description=?, start=?, end=?, location=?, cost=?, paid_by=? WHERE id=?');
if (!$stmt) {
...
if ($paidBy === 'pool' && $cost > 0) {
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->fetch()) {
                $stmt->close();
                $stmt = $mysqli->prepare('UPDATE reservations_v2 SET amount=?, updated_at=CURRENT_TIMESTAMP WHERE id=?');
                if ($stmt) {
