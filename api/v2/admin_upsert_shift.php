...
if ($action === 'delete') {
    ...
    $stmt->close();
    api_log_admin_action($conn, api_current_user_id() ?? 0, 'admin_delete_shift', 'shift', (string) $shiftId, []);
...
api_log_admin_action($conn, $adminId, 'admin_upsert_shift', 'shift', $date . '#' . $memberId, [
    'type' => $type,
]);
