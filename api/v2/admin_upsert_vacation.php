...
if ($action === 'delete') {
    ...
    $stmt = $mysqli->prepare('DELETE FROM vacations WHERE id=? AND member_id=?');
    if (!$stmt) {
    ...
    $stmt->close();
    api_log_admin_action($mysqli, api_current_user_id() ?? 0, 'admin_delete_vacation', 'vacation', (string) $vacationId, []);
...
if ($vacationId === null) {
    $stmt = $mysqli->prepare('INSERT INTO vacations (member_id, start_date, end_date, created_by) VALUES (?,?,?,?)');
    if (!$stmt) {
    ...
    $stmt->close();
    api_log_admin_action($mysqli, $creator, 'admin_upsert_vacation', 'vacation', (string) $newId, $data);
    api_send_response('success', ['saved' => true, 'id' => (int) $newId]);
}

$stmt = $mysqli->prepare('UPDATE vacations SET start_date=?, end_date=? WHERE id=? AND member_id=?');
if (!$stmt) {
...
api_log_admin_action($mysqli, api_current_user_id() ?? 0, 'admin_upsert_vacation', 'vacation', (string) $vacationId, $data);
