...
if (!empty($participants)) {
    $stmt = $conn->prepare('INSERT INTO event_participants (event_id, member_id, state, availability) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE state=VALUES(state), availability=VALUES(availability)');
    if ($stmt) {
...
if ($notifyDiscord) {
    ...
    $stmt = $conn->prepare('INSERT INTO discord_notifications (event_id, payload) VALUES (?,?)');
    if ($stmt) {
