...
// Mitgliederliste
$stmt = $mysqli->prepare('SELECT id, name FROM members_v2 WHERE status="active" AND is_locked=0 ORDER BY name ASC');
if (!$stmt) {
...
// Präsenzdaten
$stmt = $mysqli->prepare("SELECT member_id, status FROM discord_status_cache WHERE member_id IN ($placeholders)");
if ($stmt) {
...
// Kranktage
$stmt = $mysqli->prepare('SELECT member_id FROM sickdays WHERE start_date <= ? AND end_date >= ?');
if ($stmt) {
...
// Urlaub
$stmt = $mysqli->prepare('SELECT member_id FROM vacations WHERE start_date <= ? AND end_date >= ?');
if ($stmt) {
    $stmt->bind_param('ss', $date, $date);
    if ($stmt->execute()) {
...
$stmt = $mysqli->prepare('SELECT member_id FROM shifts WHERE shift_date = ?');
if ($stmt) {
...
$stmt = $mysqli->prepare("SELECT member_id FROM shifts WHERE shift_date = ? AND type='night'");
if ($stmt) {
