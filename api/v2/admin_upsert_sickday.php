...
if ($action === 'delete') {
    ...
    $stmt->close();
    api_log_admin_action($mysqli, api_current_user_id() ?? 0, 'admin_delete_sickday', 'sickday', (string) $sickId, []);
...
$stmt = $mysqli->prepare('SELECT id FROM vacations WHERE member_id=? AND NOT (end_date < ? OR start_date > ?) LIMIT 1');
if ($stmt) {
    $stmt->bind_param('iss', $memberId, $start, $end);
...
if ($sickId === null) {
    ...
    $stmt->close();
    api_log_admin_action($mysqli, $creator, 'admin_upsert_sickday', 'sickday', (string) $newId, $data);
...
api_log_admin_action($mysqli, api_current_user_id() ?? 0, 'admin_upsert_sickday', 'sickday', (string) $sickId, $data);
