...
$total = 0;
$stmt = $conn->prepare('SELECT COUNT(*) FROM members_v2');
...
$stmt = $conn->prepare('SELECT id, name, COALESCE(email, ""), COALESCE(discord_tag, ""), COALESCE(avatar_url, ""), COALESCE(roles, "member"), status, joined_at, left_at, is_locked FROM members_v2 ORDER BY created_at DESC LIMIT ?, ?');
if (!$stmt) {
