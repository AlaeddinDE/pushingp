...
try {
    ...
    $member = fetch_member_by_name($mysqli, $member_name);
    if ($member === null) {
    ...
    $creator = fetch_member_by_name($mysqli, $user);
    $creatorId = $creator['id'] ?? $member['id'];

    // Füge Schicht hinzu
    $stmt = $mysqli->prepare("INSERT INTO shifts (member_id, member_name, shift_date, shift_start, shift_end, start_time, end_time, shift_type, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
