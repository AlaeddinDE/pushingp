...
if ($lock) {
    $stmt = $conn->prepare("UPDATE members SET is_locked = 1, locked_at = NOW(), locked_reason = ? WHERE name = ?");
    if (!$stmt) {
        json_response(['error' => 'Benutzer konnte nicht gesperrt werden'], 500);
    }
    $stmt->bind_param('ss', $reason, $name);
} else {
    $stmt = $conn->prepare("UPDATE members SET is_locked = 0, locked_at = NULL, locked_reason = NULL WHERE name = ?");
    if (!$stmt) {
