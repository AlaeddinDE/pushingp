...
if ($action === 'add') {
  ...
  $stmt = $mysqli->prepare('INSERT INTO admins (pin, member_name) VALUES (?, ?) ON DUPLICATE KEY UPDATE member_name = ?');
  ...
  $stmt = $mysqli->prepare('UPDATE admins SET member_name = NULL WHERE member_name = ?');
  $stmt->bind_param('s', $member_name);
}
